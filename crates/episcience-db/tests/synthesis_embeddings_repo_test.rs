use episcience_db::{SynthesisRepository, SynthesisEmbeddingsRepository};
...
async fn upsert_and_exists(pool: PgPool) {
    ...
    assert!(!SynthesisEmbeddingsRepository::exists(&pool, synthesis_id).await.unwrap());

    SynthesisEmbeddingsRepository::upsert(
        &pool, synthesis_id, &emb, "text-embedding-3-small", "narrative_head",
    ).await.unwrap();
...
async fn upsert_nonexistent_synthesis_fails(pool: PgPool) {
    ...
    let result = SynthesisEmbeddingsRepository::upsert(
        &pool, Uuid::now_v7(), &emb, "text-embedding-3-small", "narrative_head",
    ).await;
