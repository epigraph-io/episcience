...
async fn readable_by_predicate_owner_can_read(pool: PgPool) {
    ...
    .await
    .unwrap();
    assert!(SynthesisRepository::readable_by(&pool, id, owner)
        .await
        .unwrap());
}
...
async fn readable_by_predicate_stranger_blocked(pool: PgPool) {
    ...
    .await
    .unwrap();
    assert!(!SynthesisRepository::readable_by(&pool, id, stranger)
        .await
        .unwrap());
}
...
async fn save_narrative_marks_complete(pool: PgPool) {
    ...
    .unwrap();
    let hash = [1u8; 32];
    SynthesisRepository::save_narrative(&pool, id, "narrative text", &hash)
        .await
        .unwrap();
    let s = SynthesisRepository::get_by_id(&pool, id).await.unwrap();
...
async fn readable_by_predicate_shared_recipient_can_read(pool: PgPool) {
    ...
    SynthesisSharesRepository::grant(&pool, id, recipient, owner)
        .await
        .unwrap();
    assert!(SynthesisRepository::readable_by(&pool, id, recipient)
        .await
        .unwrap());
}
