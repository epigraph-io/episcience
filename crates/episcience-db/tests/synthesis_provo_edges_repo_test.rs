use episcience_db::{SynthesisRepository, SynthesisProvoEdgesRepository};
...
async fn plan_and_list_pending(pool: PgPool) {
    ...
    SynthesisProvoEdgesRepository::plan(&mut tx, synthesis_id, &edges).await.unwrap();
    tx.commit().await.unwrap();

    let pending = SynthesisProvoEdgesRepository::list_pending(&pool, synthesis_id).await.unwrap();
    assert_eq!(pending.len(), 2);

    let count = SynthesisProvoEdgesRepository::count_pending(&pool, synthesis_id).await.unwrap();
    assert_eq!(count, 2);
}
...
async fn mark_written_removes_from_pending(pool: PgPool) {
    ...
    let mut tx = pool.begin().await.unwrap();
    SynthesisProvoEdgesRepository::plan(&mut tx, synthesis_id, &[e.clone()]).await.unwrap();
    tx.commit().await.unwrap();

    let edge_id = Uuid::now_v7();
    SynthesisProvoEdgesRepository::mark_written(
        &pool, synthesis_id, &e.predicate, &e.target_kind, e.target_id, edge_id,
    ).await.unwrap();
...
async fn record_failure_stores_error(pool: PgPool) {
    ...
    SynthesisProvoEdgesRepository::record_failure(
        &pool, synthesis_id, &e.predicate, &e.target_kind, e.target_id, "connection error",
    ).await.unwrap();
